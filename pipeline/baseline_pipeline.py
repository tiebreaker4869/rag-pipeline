...
class SimpleRAGPipeline:
    def __init__(self, vision_embedding_dir: str, doc_dir: str, chunk_size: int, chunk_overlap: int, vision_topk: int, keyword_k: int, dense_k: int, final_k: int, hybrid_weights: List[float], model: str):
    ...
    def query(self, question: str):
        pages = self.vision_retriever.retrieve(question, self.vision_topk) # every page result is a dict with keys: doc_id, page_num, score
        ...
        self.text_retriever = HybridRetriever(self.keyword_k, self.dense_k, page_documents)
        retrieved_documents = self.text_retriever.retrieve(question, self.final_k, self.hybrid_weights)
...
def main():
    ...
    rag = SimpleRAGPipeline(args.embedding_dir, args.doc_dir, args.chunk_size, args.chunk_overlap, args.vision_k, args.keyword_k, args.dense_k, args.final_k, [args.keyword_weight, 1 - args.keyword_weight], args.generation_model)
