...
class Retriever:
    def __init__(self, doc_dir: str, embedding_dir: str = None, top_k: int = 5):
        ...
        self.processor_emb = ColQwen2_5_Processor.from_pretrained(
        ...
        self.embedding_files = glob(
            os.path.join(self.embedding_dir, "**/*.pt"), recursive=True
        )
        print(
            f"[INFO] Found {len(self.embedding_files)} embedding files in {self.embedding_dir}"
        )
    ...
    def retrieve(self, question: str, top_k: int = None) -> List[Dict[str, any]]:
        ...
        for emb_path in self.embedding_files:
            ...
            try:
                ...
                with torch.no_grad():
                    scores = self.processor_emb.score_multi_vector(
                ...
                for page_num, score in enumerate(page_scores, start=1):
                    all_page_scores.append(
                        {"doc_id": doc_id, "page_num": page_num, "score": score}
                    )
        ...
        for i, result in enumerate(top_results):
            print(
                f"  {i+1}. {result['doc_id']} - Page {result['page_num']}: {result['score']:.4f}"
            )
