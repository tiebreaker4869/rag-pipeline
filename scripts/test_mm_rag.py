#!/usr/bin/env python3
"""Test script for MultimodalRAGPipeline."""
...
from rag_pipeline.pipelines import MultimodalRAGPipeline
from rag_pipeline.utils.profile import export_latency, get_all_metrics


def main():
    parser = argparse.ArgumentParser(
        description="Test MultimodalRAGPipeline with interactive queries"
    )
    parser.add_argument(
        "--doc_dir",
        type=str,
        required=True,
        help="Directory containing .pt embeddings and markdown files",
    )
    parser.add_argument(
        "--vision_top_k",
        type=int,
    ...
    print("=" * 80)
    print("MultimodalRAGPipeline Test Script")
    print("=" * 80)
    ...
    # Initialize pipeline
    print("\n[INFO] Initializing MultimodalRAGPipeline...")
    try:
    ...
    csv_path = os.path.join(args.output_dir, "mm_rag_metrics.csv")
    json_path = os.path.join(args.output_dir, "mm_rag_metrics.json")
