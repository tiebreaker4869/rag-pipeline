"""Multimodal RAG pipeline with LLM-based reranking.

Stage 1: ColPali vision-based page retrieval
Stage 2: Text-based chunk retrieval on retrieved pages
Stage 3: LLM-based reranking (returns all relevant chunks)
...
from rag_pipeline.rerankers import LLMReranker
...
class MultimodalRAGLLMRerankPipeline(BaseRAGPipeline):
    """Multimodal RAG with LLM-based reranking: ColPali -> text retrieval -> LLM rerank -> generation.
    ...
    def __init__(
        ...
        rerank_llm_model: str = "gemini-2.5-flash-lite",
        rerank_top_k: Optional[int] = None,
    ):
        """Initialize multimodal RAG pipeline with LLM reranking.
        ...
        Args:
            ...
            llm_model: LLM model name for final generation
            embedding_model: Embedding model name for text retrieval
            rerank_llm_model: LLM model for reranking (default: gemini-2.0-flash-thinking-exp-01-21)
            rerank_top_k: Optional limit on reranked chunks (None returns all relevant)
        """
        ...
        print(f"[INFO] Initializing LLM reranker with model: {rerank_llm_model}")
        rerank_llm = GeminiChat(model=rerank_llm_model)
        self.reranker = LLMReranker(llm=rerank_llm, temperature=0.0)

        print(f"[INFO] Multimodal LLM rerank pipeline initialized")
    ...
    def query(self, question: str) -> str:
        """Run multimodal RAG query with LLM reranking.
    ...
    def query_with_metadata(self, question: str) -> RAGResponse:
        ...
        # Stage 4: LLM-based reranking
        with latency_context("LLMRerank"):
            retrieved_documents = self.reranker.rerank(
                question, retrieved_documents, top_k=self.rerank_top_k
            )

        # Extract final pages (after reranking)
        final_pages = self._extract_page_numbers(retrieved_documents)

        print(f"[INFO] After LLM reranking: {len(retrieved_documents)} relevant chunks from pages {final_pages}")
